...
| `cardProps`<el-tag>v0.1.1</el-tag> | 分组表单 el-card 的 props，`group`存在时生效 ，优先级低于`group`配置的每一项的`cardProps` [PlusFormGroupRow](/components/type.html#plusformgrouprow) | `object` [ElCardProps](https://element-plus.org/zh-CN/component/card.html#attributes)    | `false` | 否       |
